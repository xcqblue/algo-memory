...
const SYNONYMS: Record<string, string[]> = {
  ...
  '宁德': ['宁德时代', '300750'],
  '比亚迪': ['BYD', '002594', '比亚迪汽车'],
  '苹果': ['Apple', 'AAPL', '苹果公司'],
  // 补充：上方各分类未覆盖的条目（无重复key）
  '基准利率': ['央行利率', '政策利率', '利率', '加息', '升息'],
  '上调': ['加息', '升息', '提高利率', '加息上调'],
  '联邦': ['美联储', 'FOMC', '美国'],
...
function extractSynonymTokensFromChinese(text: string): string[] {
  ...
  // 递归提取：key 匹配时，同时也把那些以当前 key 为 value 的反向 key 纳入
  function addKeyAndRelated(key: string): void {
    ...
    // 反向查找：以当前 key 为 value 的其他 key（双向展开）
    for (const [otherKey, otherVals] of Object.entries(SYNONYMS)) {
      if (otherVals.includes(key) && !visited.has(otherKey)) {
        addKeyAndRelated(otherKey);
      }
    }
  }
...
export function simpleChineseTokenize(text: string): string[] {
  ...
  for (const seg of segments) {
    ...
    if (seg.lang === 'latin') {
      ...
      // 纯中文或混合段（seg.lang === 'cjk'）
      // 先用 SYNONYMS 做子串提取（解决"做T"等混合token）
