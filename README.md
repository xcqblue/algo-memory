...
- **同义词扩展检索**（离线，零依赖）— Query 时将 token 展开为 OR 查询，覆盖同义/相关词
  - 脚本感知分词：Latin 与 CJK（中文/日文）分段处理
  - SYNONYMS 双向子串提取：自动提取命中同义词表的 2-gram/3-gram 子串
  - 支持场景：生活（老婆↔媳妇、iPhone↔苹果手机）、金融（止损↔割肉、加仓↔增持）、宏观（CPI↔通胀、美联储↔FOMC）等
- **多路召回**（Multi-Path）— 同一 Query 生成多条检索路径，合并去重后统一 MMR
  - 路径1：原始 Query
  - 路径2：前缀 3-token
  - 路径3：后缀 2-token
  - 路径4：首尾 token 组合
- **BM25 排序**，支持 BM25+ 评分增强（δ=1.0）
...
| `dedupThreshold` | `0.85` | Jaccard 相似度阈值；双方或一方为元数据内容时自动降低 |
...
| `noiseFilter.skipPatterns` | 见下方 | 正则数组，符合任一 pattern 的内容直接跳过 |
| `noiseFilter.skipSystemSource` | `true` | 预留：跳过系统/元数据来源的消息（来源标签过滤） |
